...
namespace GaussianSplatting.Runtime
{
    ...
    public class GpuSortingRadix : GpuSorting
    {
        ...
        public class SupportResourcesRadix : GpuSorting.SupportResources
        {
            ...
            public static SupportResourcesRadix Load(uint count)
            {
                ...
                var resources = new SupportResourcesRadix
                {
        ...
        public GpuSortingRadix(ComputeShader cs)
        {
            m_CS = cs;
            if (cs)
            {
        ...
        public override void Dispatch(CommandBuffer cmd, Args args)
        {
            ...
            SupportResourcesRadix resources = args.resources as SupportResourcesRadix;

            GraphicsBuffer srcKeyBuffer = args.inputKeys;
            GraphicsBuffer srcPayloadBuffer = args.inputValues;

            GraphicsBuffer dstKeyBuffer = resources.altBuffer;
            GraphicsBuffer dstPayloadBuffer = resources.altPayloadBuffer;
            ...
            //Upsweep
            cmd.SetComputeBufferParam(m_CS, m_kernelUpsweep, "b_passHist", resources.passHistBuffer);
            cmd.SetComputeBufferParam(m_CS, m_kernelUpsweep, "b_globalHist", resources.globalHistBuffer);

            //Scan
            cmd.SetComputeBufferParam(m_CS, m_kernelScan, "b_passHist", resources.passHistBuffer);

            //Downsweep
            cmd.SetComputeBufferParam(m_CS, m_kernelDownsweep, "b_passHist", resources.passHistBuffer);
            cmd.SetComputeBufferParam(m_CS, m_kernelDownsweep, "b_globalHist", resources.globalHistBuffer);

            //Clear the global histogram
            cmd.SetComputeBufferParam(m_CS, m_kernelInitDeviceRadixSort, "b_globalHist", resources.globalHistBuffer);
